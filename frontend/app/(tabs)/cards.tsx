...
export default function Cards() {
  ...
  const [spendingInsights, setSpendingInsights] = useState<SpendingInsight | null>(null);
  ...
  const getCardColors = (cardName: string): [string, string] => {
    ...
    const matchedColor = Object.entries(colorMap).find(([key]) => lowerName.includes(key));
  ...
  const getBestForCategories = (subtype: string): string[] => {
    ...
    return categoryMap[subtype?.toLowerCase()] || 
           ['Everyday Purchases', 'Dining', 'Shopping'];
  };
          ...
          <>
                ...
                {cards.map((card, index) => (
                  <View key={`${card.id}-${index}`}>
                    ...
                    >
                      ...
                      {card.utilization !== null && card.utilization !== undefined && (
                            ...
                            <View 
                              style={[
                                ...
                                { 
                                  ...
                                  backgroundColor: card.utilization > 80 ? '#EF4444' : '#10B981'
                                }
                              ]} 
                            />
                          </View>
            ...
            <View>
              ...
              <View style={styles.recBox}>
                {spendingInsights?.topCategories?.slice(0, 4).map((item, index) => (
                  <View
                    key={`${item.category}-${index}`}
                    style={[
                      styles.recRow,
                      index < Math.min(3, (spendingInsights?.topCategories?.length || 0) - 1) && styles.recRowBorder,
                    ]}
                  >
                    <View style={{ flex: 1 }}>
                      <Text style={styles.recCategory}>{item.category}</Text>
                      <Text style={styles.recSub}>Earn {item.percentage}% more</Text>
                    </View>
            ...
            <View style={styles.totalBox}>
              ...
              <Text style={styles.totalValue}>
                ${cards.reduce((sum, card) => sum + parseFloat(card.rewards.cashback || '0'), 0).toFixed(2)}
              </Text>
...
const styles = StyleSheet.create({
  ...
  recCategory: { fontWeight: "600", color: "#111827", marginBottom: 2, fontSize: 14 },
  ...
  totalTitle: { fontWeight: "600", color: "#111827", marginBottom: 6, fontSize: 16 },
  totalValue: {
    fontSize: 32,
    fontWeight: "700",
    color: "#111827",
