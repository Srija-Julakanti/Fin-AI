...
import { ChevronRight, User, Bell, Shield, CreditCard, Users, HelpCircle, LogOut, Moon, Sun, Monitor } from 'lucide-react';
  ...
  const settingsSections : SettingsSection[] = [
    {
      ...
      items: [
        ...
        { icon: CreditCard, label: 'Linked Accounts', badge: '3', hasArrow: true },
    ...
    },
    {
      ...
      items: [
        ...
        { icon: Shield, label: 'Biometric Login', hasSwitch: true, enabled: true },
  ...
  return (
    <div className="min-h-screen p-6 space-y-6">
      ...
      <div className="space-y-1">
        ...
        <p className="text-slate-600 dark:text-slate-400">Manage your account</p>
      </div>
      ...
      {settingsSections.map((section, sectionIndex) => (
        <div key={sectionIndex} className="space-y-3">
          <h3 className="text-slate-800 dark:text-slate-100">{section.title}</h3>
              ...
              {section.items.map((item, itemIndex) => (
                <div key={itemIndex}>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <p className="text-slate-900 dark:text-slate-100">{item.label}</p>
                    </div>
                    {item.badge && (
                    ...
                    {item.hasArrow && <ChevronRight className="w-5 h-5 text-slate-400 dark:text-slate-500" />}
                  </div>
      ...
      <FinAICard className="bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-200 dark:border-blue-800">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-slate-900 dark:text-slate-100">Bank-Level Security</p>
            <p className="text-slate-600 dark:text-slate-400 mt-1">All your financial data is encrypted with 256-bit encryption and never shared without your permission.</p>
          </div>
        </div>
      </FinAICard>
      ...
      {/* App Version */}
      <p className="text-center text-slate-400 dark:text-slate-500">Fin-AI v1.0.0</p>
    </div>
  );
}
