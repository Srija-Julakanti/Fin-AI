...
export default function CreateBudgetScreen({ onBack, onSuccess, onClose }: CreateBudgetScreenProps) {
  ...
  const categories = [
    ...
    { id: 'transportation', label: 'Transportation', icon: '🚗', suggested: 300 },
    ...
    <div className="min-h-screen p-6 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-slate-800 dark:text-slate-100">Create Monthly Budget</h1>
          <p className="text-slate-600 dark:text-slate-400">Set spending limits for each category</p>
        </div>
      </div>
      ...
      <FinAICard className="bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 border-2 border-purple-200 dark:border-purple-800">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 space-y-2">
            <p className="text-slate-900 dark:text-slate-100">AI Budget Suggestions</p>
            <p className="text-slate-600 dark:text-slate-400">
              Based on your income and spending patterns, we recommend a monthly budget of ${categories.reduce((sum, cat) => sum + cat.suggested, 0).toLocaleString()}.
            </p>
            <Button 
              size="sm" 
          ...
          <div className="space-y-4">
            {categories.map((category) => (
              <div key={category.id} className="space-y-2">
                <Label htmlFor={category.id} className="dark:text-slate-200 flex items-center gap-2">
