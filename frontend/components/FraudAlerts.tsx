  ...
  return (
    <div className="min-h-screen p-6 space-y-6">
      ...
      <div className="space-y-1">
        ...
        <p className="text-slate-600 dark:text-slate-400">Real-time security monitoring</p>
      </div>
        ...
        <FinAICard className="bg-gradient-to-r from-red-500 to-orange-500 text-white">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-white/90">{pendingCount} suspicious {pendingCount === 1 ? 'transaction' : 'transactions'} need your review</p>
            </div>
          </div>
        </FinAICard>
      )}
        ...
        {alerts.map((alert, index) => (
          <FinAICard 
            ...
            className={`bg-white dark:bg-slate-800 ${
              alert.status === 'pending' ? 'border-2 border-orange-300 dark:border-orange-700' : ''
            }`}
          >
            <div className="space-y-3">
              {/* Header */}
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <p className="text-slate-900 dark:text-slate-100">{alert.merchant}</p>
                  ...
                  <p className="text-slate-600 dark:text-slate-400">{alert.description}</p>
                </div>
                <p className="text-slate-900 dark:text-slate-100 text-xl">${alert.amount}</p>
              </div>
      ...
      <FinAICard className="bg-gradient-to-br from-blue-50 to-teal-50 dark:from-blue-900/20 dark:to-teal-900/20 border-2 border-blue-200 dark:border-blue-800">
        <div className="space-y-2">
          ...
          <p className="text-slate-600 dark:text-slate-400">We monitor your accounts 24/7 using advanced machine learning to detect suspicious activity in real-time.</p>
        </div>
      </FinAICard>
    </div>
  );
